...
async function dexLogoClicked() {
  await fetchDexData(lastSelectedWord).then((response) => {
    if (!response.definitions.length) {
      ...
      definitionBoxData.def = response.definitions[0].htmlRep;
    }
  });
...
///////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////
async function fetchDexData(data1) {
  const cachedDefinition = await chrome.storage.local.get([
    "definition",
    "word",
  ]);

  if (cachedDefinition.word === data1) {
    console.log("Word retrieved from cache");

    return cachedDefinition.definition;
  } else {
    var myurl = "https://dexonline.ro/definitie/" + data1 + "/json";
    const definition = fetch(myurl)
      .then(function (response) {
        console.log("request done");
        return response.json();
      })
      .then(function (data) {
        chrome.storage.local.set({ word: data1, definition: data }, () => {
...
document.body.addEventListener("click", (event) => {
  //get the selected word, trim white spaces, delete the punctuations, to lower case.
